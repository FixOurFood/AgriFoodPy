...
prelimimary Self-Sufficiency and Import Dependency Ratios (SSR, IDR) and add new
items and years to the dataset.
We will also print the SSR and IDR values to the console.
Finally, we will scale the food balance sheet to reduce animal products and plot
the results.
"""
...
import agrifoodpy.food.model as afpm
...
#%%
# Compute preliminary Self-Sufficiency Ratio (SSR) and Import Dependency Ratio (IDR)
...
# once. To pick up where we left, we indicate which node to start execution from

# Define a year dependent linear scale starting decreasing at 2021 from 1 to 0.5
